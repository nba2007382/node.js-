...
export class CookieService {
  ...
  async getcookie () {
    ...
    let w: number;
    const c = 100;
    let obj = await getTid(this.axios);
    if(obj.retcode === 20000000){
        t = obj.data.tid;
        w = obj.data.new_tid === true ? 3:2;
    };
    const res = await lastValueFrom(
      this.axios.get(`https://passport.weibo.com/visitor/visitor?t=${t}&w=${w}&a=${a}&cb=${cb}&from=${from}&c=${c}`)
    );
