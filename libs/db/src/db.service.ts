...
export class DbService {
  constructor(
    ...
    @InjectModel(Unprice)
    private readonly unprice: ReturnModelType<typeof Unprice>,
    @InjectModel(Monito_BeiKe)
    private readonly monito_BeiKe: ReturnModelType<typeof Monito_BeiKe>,
    @InjectModel(Monito_Jd)
    private readonly monito_Jd: ReturnModelType<typeof Monito_Jd>,
    ...
    for (let i = 0; i < type.length; i++) {
      ...
      for (let j = 0; j < stop; j++) {
        setTimeout(async () => {
          const t = j * 20;
          const url =
            'https://movie.douban.com/j/new_search_subjects?sort=S&range=0,10&tags=&start=' +
            t +
            '&countries=' +
            countries;
  ...
  async monito_WeiBoUpdata() {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      for (let i = 0; i < page; i++) {
        const updataUrl =
          'https://weibo.com/ajax/statuses/mymblog?uid=' +
          id +
          '&page=' +
          page +
          '&feature=0';
  ...
  async monito_JdUpdata() {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      await this.monito_Jd.updateMany(
        { id: el.id },
  ...
  async beiKe_houseListUpdata() {
    ...
    for (let j = 0; j < type.length; j++) {
      ...
      for (let i = 1; i < stopNum; i++) {
        const url =
          'https://zh.ke.com/ershoufang/' + County + '/pg' + i + 'co42/';
        await new Promise((resolve) => {
          setTimeout(async () => {
            ...
            $('.sellListContent .clear').each(function () {
              if (
                $('.VIEWDATA .lj-lazy', this).attr('data-original') !==
                undefined
              ) {
                const pattern = /housedel_id=\d+/;
                const num = /\D+/;
                let housedel_id;
                eval(
                  pattern.exec(
                    $('.info .title a', this).attr('data-action'),
                  )[0],
                ); //housedel_id
                const name =
                  County == 'xiangzhouqu'
                ...
                const position = $(
                  '.info .address .positionInfo a',
                  this,
                ).text(); //房子位置
                const houseIcon = $('.info .address .houseInfo ', this).text(); //房屋简要信息
                const price = parseInt(
                  $('.info .address .priceInfo .totalPrice span', this).text(),
                );
                const up = $(
                  '.info .address .priceInfo .unitPrice span',
                  this,
                ).text();
                const untiprice = parseInt(
                  up.replace(num, '').replace(num, ''),
                ); //均价
                allFilms.push({
                  housedel_id,
                  name,
    ...
    for (let i = 0; i < allFilms.length; i++) {
      const el = allFilms[i];
      await this.beiKe.updateMany(
        { housedel_id: el.housedel_id },
        {
          name: el.name,
          title: el.title,
          href: el.href,
          ...
          houseIcon: el.houseIcon,
          price: el.price,
          untiprice: el.untiprice,
        },
        {
          upsert: true,
        },
