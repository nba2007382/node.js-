...
export class BeiKeService {
  constructor(
    ...
    @InjectModel(Unprice)
    private readonly beike_Unprice: ReturnModelType<typeof Unprice>,
  ...
  async createMonitoBeiKe(
    monitoUrl: string,
    monitoId: string,
    accountEmail: string,
  ) {
    const res = await lastValueFrom(this.axios.get(monitoUrl));
    const html = res.data + '';
    const data = [];
  ...
  async delMonitoBeiKe(monitoId: string, accountEmail: string) {
    ...
    if (length === 1) {
      ...
      await this.monito_BeiKe.updateOne(
        { id: monitoId },
  ...
  async getChartList() {
    const chart1 = await this.beike_Unprice.aggregate([
      { $sort: { create_time: -1 } },
      { $limit: 3 },
      { $sort: { name: -1 } },
      {
        $group: {
          _id: 0,
          name: { $push: '$name' },
          unprice: { $push: '$untiprice' },
        },
      },
    ]);
    const chart2 = await this.beike_Unprice.aggregate([
      { $sort: { create_time: 1 } },
      { $limit: 21 },
      { $sort: { name: -1 } },
      {
